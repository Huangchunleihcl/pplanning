...
import numpy as np # 'NumPyBackend' object has no attribute 'integer','floating','ndarray'
...
class PerformanceEvaluator:
    ...
    def evaluate_all(self, algorithms=None, n_trials=30):
        ...
        tasks = [(case, algo) for case in self.test_cases for algo in algorithms]  # 创建任务列表（每个地图一个任务）
               
        with ProcessPoolExecutor(max_workers=os.cpu_count()//2) as executor:                             # 使用并行加速
            futures = [
                executor.submit(self._evaluate_single_task, case, algo, n_trials)
    ...
    def _evaluate_single_task(self, case, algo, n_trials):
        """单个评估任务（地图+算法）"""
